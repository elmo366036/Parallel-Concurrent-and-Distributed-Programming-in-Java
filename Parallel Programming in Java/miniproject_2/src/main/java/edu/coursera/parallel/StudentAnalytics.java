...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
    	...
    	String mostCommon = Stream.of(studentArray)
    			.parallel()
         		.filter(s -> !s.checkIsCurrent()) 	//can't use method reference
         		.map(Student::getFirstName)			//using method reference
         		.collect(Collectors.groupingBy(x -> x, Collectors.counting())) 
         											// creates a map of {firstName : count}
         											// had to use Collectors. for this to work   
         											// groups by first name and count into a map
         		.entrySet()							//returns the mappings just created
         		.parallelStream() 					//parallelizes them
         		.max(Map.Entry.comparingByValue())	//looks for max value
         		.get()								//gets it
         		.getKey();							//returns the key (the first name) mapped to the max value
    	return mostCommon;
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
    	...
    	int retVal = (int) Stream.of(studentArray)
    			.parallel()
         		.filter(s -> !s.checkIsCurrent() && s.getAge() > 20 && s.getGrade() < 65) 	//can't use method reference
         		.count();
    	return retVal;
    }
}
